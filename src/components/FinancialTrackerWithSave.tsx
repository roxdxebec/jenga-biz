...
interface FinancialTrackerWithSaveProps {
  language?: string;
  currency?: string;
  currencySymbol?: string;
}

const FinancialTrackerWithSave = ({ 
  language = 'en', 
  currency = 'KES', 
  currencySymbol = 'KSh' 
}: FinancialTrackerWithSaveProps) => {
  ...
  // Load existing financial records
  useEffect(() => {
    if (user) {
  ...
  const loadFinancialRecords = async () => {
    ...
    try {
      // For now, we'll use local storage since financial_records requires business_id
      // In a full implementation, we'd need to link to a business first
      const savedTransactions = localStorage.getItem(`transactions_${user.id}`);
      if (savedTransactions) {
        const parsed = JSON.parse(savedTransactions);
        const formattedTransactions = parsed.map((record: any) => ({
          ...record,
  ...
  const saveToDatabase = async (transaction: Transaction) => {
    if (!user) return;
    
    setIsSaving(true);
    try {
      // Save to localStorage for now since financial_records requires business_id
      ...
      toast({
        title: "Saved",
        description: "Financial record saved successfully"
      });
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Alert className="mb-6 bg-blue-50 border-blue-200 max-w-2xl mx-auto">
          ...
          <AlertDescription className="text-blue-800">
            <strong>How it works:</strong> Record revenue and expenses in any order. Take a photo of receipt or upload image/screenshot from your storage, or enter details manually.
          </AlertDescription>
        </Alert>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  disabled={isSaving}
                >
                  {isSaving ? 'Saving...' : 'Record New Payment'}
                </Button>
                <Button 
                  variant="outline" 
                  size="icon"
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  disabled={isSaving}
                >
                  {isSaving ? 'Saving...' : 'Record New Expense'}
                </Button>
                <Button 
                  variant="outline" 
                  size="icon"
...
export default FinancialTrackerWithSave;